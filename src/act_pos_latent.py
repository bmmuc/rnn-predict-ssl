...
class ActLatent(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        encode = self.act_autoencoder.encoding(x)

        encode = encode.to(device)

        out = self.model['linear1'](encode)
        ...
        # decode = self.pos_autoencoder.decoding(out, x_hist_copy)

        # decode = decode.view(decode.shape[0], decode.shape[2], decode.shape[1])

        return out

    def encoding(self, x):
        # ipdb.set_trace()
        encoded = self.act_autoencoder.encoding(x)
        return encoded
    ...
    def decoding(self, x, y_hist):
        decoded = self.pos_autoencoder.decoding(x, y_hist)
        return decoded
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict({ 'train/loss/general_loss_val_latent': general_loss, 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict({ 'val/loss/general_loss_val_latent': general_loss, 
