...
class AttnDecoder(nn.Module):
    ...
    def forward(self, input_encoded: torch.Tensor, y_history: torch.Tensor):
        ...
        for t in range(self.seq_len):
            ...
            xew = torch.bmm(x.unsqueeze(1), input_encoded.to(device))[
                :, 0, :]  # (batch_size, encoder_hidden_size)

            y_tilde = self.fc(torch.cat((xew.to(device), y_history[:, t].to(device)),
        ...
        out = self.fc_out(torch.cat((h_t[0], xew.to(device)), dim=1))
