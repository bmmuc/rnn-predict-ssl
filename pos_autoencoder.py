...
class AttnEncoder(nn.Module):
    ...
    def _get_noise(self, input_data: torch.Tensor, sigma=0.01, p=0.15):
        """
        Get noise.
        Args:
...
class AttnDecoder(nn.Module):
    ...
    def forward(self, input_encoded: torch.Tensor, y_history: torch.Tensor):
        ...
        for t in range(self.seq_len):
            ...
            con = torch.bmm(x.unsqueeze(1), input_encoded.to(device))[
                :, 0, :]  # (batch_size, encoder_hidden_size)

            y_tilde = self.fc(torch.cat((con.to(device), y_history[:, t].to(device)),
...
class PositionAutoEncoder(nn.Module):
    ...
    def training_step(self, X, y):
        ...
        # ipdb.set_trace()
        tolerance = 1e-6
        indices = torch.gt(pred, 1 - tolerance)
